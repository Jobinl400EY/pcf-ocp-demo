...
app='enterprise-app'
appname="$app-$rand1"
cfdomain='20.231.121.184.nip.io'
echo -e "\nappname is: $appname"
...
$CF_CLI_TOOL set-env gateway ENTERPRISE_APP_CUSTOMERS_URL "http://$appname-customers.$cfdomain/customers"
$CF_CLI_TOOL set-env gateway ENTERPRISE_APP_INVENTORY_URL "http://$appname-inventory.$cfdomain/products"
$CF_CLI_TOOL set-env gateway ENTERPRISE_APP_ORDERS_URL "http://$appname-orders.$cfdomain/orders"
...
$CF_CLI_TOOL push --no-start -t 180 -s cflinuxfs3 --hostname "$appname"-frontend
$CF_CLI_TOOL set-env frontend ENTERPRISE_APP_GATEWAY_URL "http://$appname-gateway.$cfdomain"
...
echo -e "\ninventory:"
echo "curl http://$appname-inventory.$cfdomain/products"

echo -e "\ncustomers:"
echo "curl http://$appname-customers.$cfdomain/customers"

echo -e "\norders:"
echo "curl http://$appname-orders.$cfdomain/orders"

echo -e "\ngateway:"
echo "curl http://$appname-gateway.$cfdomain/customers"
echo "curl http://$appname-gateway.$cfdomain/orders"
echo "curl http://$appname-gateway.$cfdomain/products"

echo -e "\nfrontend:\nVisit http://$appname-frontend.$cfdomain URL on your browser. Please refresh the page if it appears blank."
